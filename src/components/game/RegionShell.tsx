    ...
    <div className="min-h-screen pb-16">
      ...
      <main className="mx-auto max-w-xl px-4 py-6">
        ...
        <ul className="mt-5 space-y-4">
          {modules.map((m, i) => (
            <li key={m.id} className={i % 2 === 0 ? "pr-4 sm:pr-12" : "pl-4 sm:pl-12"}>
              <Link
                to={m.to}
                className={`flex items-center gap-4 rounded-4xl px-5 py-5 toy-press ${m.color}`}
              >
                <span className="text-4xl">{m.emoji}</span>
                <span className="min-w-0 flex-1">
                  <span className="block font-display text-xl leading-tight">{m.title}</span>
